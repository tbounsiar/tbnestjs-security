...
export function doRequest(
  ...
  agent.end((error, response) => {
    ...
    if (output.unauthorized || output.forbidden) {
      expect(
        output.unauthorized ? response.unauthorized : response.unauthorized
      );
      ...
      if (output.body) {
        expect(response.body).toStrictEqual(output.body);
      }
      if (output.text) {
    ...
    if (output.setCookies) {
      ...
      for (const setCookie of output.setCookies) {
        const cookie = response
          .get('Set-Cookie')
          .find((c) => c.startsWith(`${setCookie.key}=`));
...
function buildDescription(input: Input, output: Output, name?: string) {
  ...
  const description = [
    name,
    `When Request ${input.method.toUpperCase()}(${input.url})`
  ];
  if (input.headers) {
  ...
  if (output.unauthorized || output.forbidden) {
    description.push(
      `\tError To Be ` + (output.unauthorized ? 'Unauthorized' : 'Forbidden')
    );
    description.push(
      `\tWith Body ` +
        JSON.stringify({
...
export function createDigetAuthenticationHeader(options?: Options) {
  const header = [`Digest realm="${REALM}"`];

  if (options?.domain) {
    header.push(`domain="${options.domain}"`);
  }
  header.push(`nonce="f58163434437bc3fb33d971645a39820"`);

  if (options?.opaque) {
    header.push(`opaque="f58163434437bc3fb33d971645a39820"`);
  }
